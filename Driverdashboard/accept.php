...
if ($ride_id) {
    ...
    $sql = "UPDATE rides SET status='accepted' WHERE id = ? AND status='pending'";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("i", $ride_id);
    $success = $stmt->execute();
} else {
    // Fallback: accept the most recent pending ride if no ID provided
    $sql = "UPDATE rides SET status='accepted' WHERE status='pending' ORDER BY id DESC LIMIT 1";
