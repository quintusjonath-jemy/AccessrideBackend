...
try {
    ...
    if ($driver_id && $ride_id) {
        $nextDriverId = $rideModel->rejectAndReassign($ride_id, $driver_id);
        
        if ($nextDriverId) {
            echo json_encode([
                "status" => "success", 
                "message" => "Ride Rejected. Reassigned to driver ID: " . $nextDriverId
            ]);
        } else {
            echo json_encode([
                "status" => "success", 
                "message" => "Ride Rejected. No other nearby drivers available."
            ]);
        }
    } else {
